...
#define NUM_OF_TYPES (160)

/* basicfwd.c: Basic DPDK skeleton forwarding example. */
static void print(int received, int total_recv, int sent, int total_sent,
					unsigned int time_passed, unsigned int pkts_counted, unsigned int pkts_type_counter[])
{
	...
	printf("\ntime passed since last check : %d", time_passed);
	printf("\nPkts/ms since last check : %d\n", (pkts_counted * 1000) / time_passed);
...
static __attribute__((noreturn)) void lcore_main(struct rte_mempool * mbuf_pool)
{
	...
	for (;;) {
		...
		{
			print(nb_rx, total_recv, nb_tx, total_sent, time_passed, pkts_counter, pkts_type_counter);
			last_time = curr_time;
			pkts_counter = 0;
		}
